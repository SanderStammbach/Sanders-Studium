...
#D= np.exp(beta*a.dag()-np.conjugate(beta)*a)

rho_c=qutip.coherent_dm(nph,alpha)

y=0

for n in range(nph):
    fock=qutip.states.fock(nph,n)
    rho_PHAV=((np.exp(-1*(alpha**2))+(alpha**(2*n))/(np.math.factorial(n)))/8200)*(fock*fock.dag())
   
    x=(np.exp(-alpha*alpha)+(alpha**(2*n))/(np.math.factorial(n)))/8200
    print(x)
    y=x+y
...
print(RHO,y)
qutip.plot_wigner_fock_distribution(1/10*rho_c+9/10*RHO,colorbar='colorbar')
plt.show()