...
\end{equation}
In the case of this calculation $\Delta U$ is zero and then the first law says that $\langleJ_{tot}\rangle=0$. Therefore it is possible to calculate the entropy production\eqref{666} only with the heat flow divide trough the temperature.
...
\begin{equation}
\langle J_{cav}\rangle=T[H_{free}\cdot \mathcal{L}_{cav}[\rho]]=2\hbar \omega_{cav}\kappa (n_f-\langle a^{\dag{}}a \rangle).
\end{equation}
...
\caption{Energy flux vs g with the parameters The parameters for the plot are $ n_h=2.6, n_c= n_f=0.02,\kappa=0.01\gamma $}
\end{figure}
...
\end{equation}
The entropy production rate is given by the formula
\begin{equation}
