...
emission).[\cite{Li2017}]. \\In paper from 1959.  Scovil and Schulz-DuBois investigated whether a laser is not
...
\caption{Schematic representation of a three-level laser heat engine
...
In a three level system the three energy levels are $E_1,E_2,E_3$. In a first part the system gets pumped from the lowest $E_1$ level to the highest level $ E_3$. The condition for the third level is that it falls to the middle level$ E2$ very quickly. On average, the system is almost not in the third state. The resonator should then have a higher decay time, so that a population inversion can build up. This means that several particles are in the energetically higher state. From this state they come almost exclusively through stimulated emission into the lower state $ E_1$. 
...
it is possible use the Lindblad-masterequation. 
In this case the master-equation get solved for steady states
...
For a good lasing is important to have a big entropy
Another problem which has not been discussed  in this report is, that the temperatures can reach the ionization temperature. 
The Fig.11 helps to get some conclusion about, when the system start to lasing and when it stops. With a too small temperature, It has not enough energy in the system and to have photons in the upper band. If $n_h$ change property, a maximum get reached. If the temperature increasing further, the system has too many photons in the upper band, the Rabi oscillation between the 0 and the $E2$ doesn't exist any more.
